...
static const uint8_t terminal_cursor[16] = {
    0x80, 0xC0, 0xE0, 0xF0,
    0xF8, 0xFC, 0xFE, 0xFF,
    0xF8, 0xD8, 0x8C, 0x0C,
    0x06, 0x06, 0x03, 0x00
...
void mouse_process_packet(void) {
    if (!mouse_state.packet_ready || !mouse_state.enabled) return;
    ...
    if (mouse_state.x < mouse_state.min_x) mouse_state.x = mouse_state.min_x;
    if (mouse_state.x > mouse_state.max_x) mouse_state.x = mouse_state.max_x;
    if (mouse_state.y < mouse_state.min_y) mouse_state.y = mouse_state.min_y;
...
bool mouse_get_cursor_visible(void) {
    return mouse_state.cursor_visible;
}