...
#include "kernel.h"   // Retained for kernel_malloc/free
#include "memutils.h" // Retained if it defines kernel_malloc/free, or if kernel.h includes it. Assuming kernel.h is sufficient.
#include "serial.h"
#include "string.h"
...
// ==================== ADAPTADOR VFS ====================
static vnode_ops_t chardev_vnode_ops = {.lookup = NULL,
                                        .create = NULL,
                                        .mkdir = NULL,
...
int chardev_vfs_read(vfs_node_t *node, uint8_t *buf, uint32_t size,
                     uint32_t offset) {
...
int chardev_vfs_write(vfs_node_t *node, const uint8_t *buf, uint32_t size,
                      uint32_t offset) {
  chardev_t *cdev = (chardev_t *)node->fs_private;
...
void chardev_vfs_release(vfs_node_t *node) {
  ...
  if (cdev) {
    ...
    // Si nadie más usa este chardev, no lo destruimos porque está en la tabla
    // global Solo lo destruiremos si es necesario (depende de tu diseño)
  }
  kernel_free(node);
}
...
vfs_node_t *chardev_to_vfs_node(chardev_t *cdev) {
  ...
  //               "chardev_to_vfs_node: Created VFS node for %s at %p\n",
  //               cdev->name, vn);
...
vfs_node_t *chardev_vfs_find(const char *name) {
  ...
  for (int i = 0; i < special_devices_count; i++) {
    if (special_devices[i] && strcmp(special_devices[i]->name, name) == 0) {
      ...
      if (node) {
        ...
        // serial_printf(COM1_BASE,
        //               "DEBUG: Created VFS node %p, device refcount=%u\n",
  ...
  // serial_printf(COM1_BASE, "ERROR: chardev_vfs_find: Device '%s' not
  // found\n",
  //               name);
  ...
  //                   special_devices[i]->name, special_devices[i]->type,
  //                   special_devices[i]->refcount);
  //   }
  // }
