...
void draw_rounded_rect(uint32_t x, uint32_t y, uint32_t w, uint32_t h, uint32_t radius, uint32_t color) {
    ...
    draw_line(x + radius, y + h - 1, x + w - radius - 1, y + h - 1, color); // Abajo
    draw_line(x, y + radius, x, y + h - radius - 1, color); // Izquierda
    draw_line(x + w - 1, y + radius, x + w - 1, y + h - radius - 1, color); // Derecha
    
    // Dibujar las esquinas redondeadas
    draw_circle_quarter(x + radius, y + radius, radius, 1, color); // Superior izquierda
    draw_circle_quarter(x + w - radius - 1, y + radius, radius, 2, color); // Superior derecha
    draw_circle_quarter(x + w - radius - 1, y + h - radius - 1, radius, 3, color); // Inferior derecha
    draw_circle_quarter(x + radius, y + h - radius - 1, radius, 4, color); // Inferior izquierda
}
...
void fill_rounded_rect(uint32_t x, uint32_t y, uint32_t w, uint32_t h, uint32_t radius, uint32_t color) {
    ...
    for (uint32_t r = 0; r < radius; r++) {
        uint32_t width = (uint32_t)(sqrt(radius*radius - (radius-r)*(radius-r)) + 0.5);
        draw_line(x + radius - width, y + r, x + radius + width, y + r, color);
        draw_line(x + radius - width, y + h - 1 - r, x + radius + width, y + h - 1 - r, color);
    }
}
...
void put_char(char c) {
    ...
    if (c == '\t') {
        g_cursor_x = (g_cursor_x + 4 * g_current_font.width) & ~(4 * g_current_font.width - 1);
    ...
    if (!glyph) return; // Carácter no soportado
...
void scroll_screen() {
    if (g_fb.bpp == 32) {
        ...
        memmove(g_fb.buffer32, 
                g_fb.buffer32 + rows_to_scroll * (bytes_per_row / 4),
                (g_fb.height - rows_to_scroll) * bytes_per_row);
        
        // Limpiar la última fila
        fill_rect(0, g_fb.height - rows_to_scroll, g_fb.width, rows_to_scroll, g_bg_color);
    } else if (g_fb.bpp == 24) {
        uint32_t bytes_per_row = g_fb.pitch;
        uint32_t rows_to_scroll = g_current_font.height;
        
        // Mover los datos de la pantalla hacia arriba
        memmove(g_fb.buffer24, 
                g_fb.buffer24 + rows_to_scroll * (bytes_per_row / 3),
                (g_fb.height - rows_to_scroll) * bytes_per_row);
...
void draw_rounded_button(uint32_t x, uint32_t y, uint32_t w, uint32_t h, uint32_t radius, const char* text, uint32_t color) {
    ...
    // Centrar el texto
    uint32_t text_width = strlen(text) * (g_current_font.width + g_current_font.spacing);
...
void draw_big_char(uint32_t x, uint32_t y, char c, uint32_t scale, uint32_t fg_color, uint32_t bg_color) {
    ...
    for (uint32_t dy = 0; dy < g_current_font.height; dy++) {
        for (uint32_t dx = 0; dx < g_current_font.width; dx++) {
            uint32_t pixel_color = (glyph[dy] & (1 << (g_current_font.width - 1 - dx))) ? fg_color : bg_color;
...
void draw_string_with_shadow(uint32_t x, uint32_t y, const char* str, uint32_t fg_color, uint32_t bg_color, uint32_t shadow_color, uint8_t shadow_offset) {
    uint32_t current_x = x;
    while (*str) {
        draw_char_with_shadow(current_x, y, *str, fg_color, bg_color, shadow_color, shadow_offset);
