...
# Copiar binarios a la imagen: primero con mtools (no requiere sudo), luego con mount
COPIED=0
if [ -d "$BIN_DIR" ] && [ -n "$(ls -A $BIN_DIR/*.elf 2>/dev/null)" ]; then
    if command -v mcopy >/dev/null 2>&1; then
        echo -e "${YELLOW}Copiando binarios con mtools...${RESET}"
        ...
        [ "$COPIED" = 1 ] && echo -e "${GREEN}Binarios añadidos a la imagen (mtools).${RESET}"
    fi
    if [ "$COPIED" = 0 ] && sudo mount -o loop "$BINFS_IMG" "$BINFS_MOUNT" 2>/dev/null; then
        echo -e "${YELLOW}Copiando binarios montando imagen (sudo)...${RESET}"
        for elf in "$BIN_DIR"/*.elf; do
            [ -f "$elf" ] || continue
            name=$(basename "$elf" .elf)
    ...
    if [ "$COPIED" = 0 ]; then
        echo -e "${YELLOW}Instala mtools (apt install mtools) o ejecuta con sudo para incluir binarios en /bin.${RESET}"
    fi
else
    echo -e "${YELLOW}No hay .elf en $BIN_DIR, imagen vacía.${RESET}"
fi
