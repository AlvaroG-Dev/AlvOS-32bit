...
bool usb_init(void) {
    ...
    terminal_printf(&main_terminal, "USB initialization complete: %u controller(s) found\r\n",
...
usb_controller_t* usb_detect_controllers(void) {
    ...
    for (uint32_t i = 0; i < pci_device_count; i++) {
        ...
        if (usb_controller_count >= USB_MAX_CONTROLLERS) {
            terminal_puts(&main_terminal, "USB: Maximum controller count reached\r\n");
            break;
        }
        ...
        switch (pci_dev->prog_if) {
            case 0x00:
                ...
                terminal_printf(&main_terminal, "Found UHCI controller at %02x:%02x.%x\r\n",
            ...
            case 0x10:
                ...
                terminal_printf(&main_terminal, "Found OHCI controller at %02x:%02x.%x (not implemented)\r\n",
                               pci_dev->bus, pci_dev->device, pci_dev->function);
                break;
                
            case 0x20:
                ctrl->type = USB_TYPE_EHCI;
                terminal_printf(&main_terminal, "Found EHCI controller at %02x:%02x.%x\r\n",
                               pci_dev->bus, pci_dev->device, pci_dev->function);
                if (ehci_init(ctrl)) {
                    usb_controller_count++;
                }
            ...
            case 0x30:
                ...
                terminal_printf(&main_terminal, "Found xHCI controller at %02x:%02x.%x (not implemented)\r\n",
                               pci_dev->bus, pci_dev->device, pci_dev->function);
                break;
                
            default:
                terminal_printf(&main_terminal, "Unknown USB controller type (prog_if=0x%02x) at %02x:%02x.%x\r\n",
...
bool usb_enumerate_device(usb_controller_t* controller, uint8_t port) {
    terminal_printf(&main_terminal, "USB: Enumerating device on controller %u, port %u...\r\n", 
                   controller->id, port);
    
    usb_device_t* device = usb_allocate_device(controller);
    if (!device) {
        terminal_puts(&main_terminal, "USB: Failed to allocate device structure\r\n");
        return false;
    }
    ...
    if (!usb_get_device_descriptor(device)) {
        terminal_puts(&main_terminal, "USB: Failed to get initial device descriptor\r\n");
        usb_remove_device(device);
        return false;
    }
    
    terminal_printf(&main_terminal, "USB: Initial descriptor: VID=%04x PID=%04x, Class=%02x, Subclass=%02x, Protocol=%02x\r\n",
                   device->descriptor.idVendor, device->descriptor.idProduct,
                   device->descriptor.bDeviceClass, device->descriptor.bDeviceSubClass,
    ...
    if (device->descriptor.bDeviceClass == USB_CLASS_HUB) {
        terminal_puts(&main_terminal, "USB: Hub detected - configuring as pass-through\r\n");
        ...
        if (usb_get_config_descriptor(device, 0, config_buffer, sizeof(config_buffer))) {
        ...
        terminal_puts(&main_terminal, "USB: Hub configured - attempting to scan hub ports\r\n");
        ...
        if (usb_control_transfer(device, &hub_desc_req, hub_desc, 9)) {
            ...
            for (uint8_t hub_port = 1; hub_port <= num_ports; hub_port++) {
                usb_setup_packet_t power_req = {
                    .bmRequestType = 0x23,  // Class, OUT, Other (port)
            ...
            for (uint8_t hub_port = 1; hub_port <= num_ports; hub_port++) {
                usb_setup_packet_t status_req = {
                    .bmRequestType = 0xA3,  // Class, IN, Other (port)
                ...
                if (usb_control_transfer(device, &status_req, &port_status, 4)) {
                    terminal_printf(&main_terminal, "USB: Hub port %u status = 0x%08x\r\n", 
                                   hub_port, port_status);
                    
                    // Bit 0 = CCS (Current Connect Status)
                    if (port_status & 0x01) {
                        terminal_printf(&main_terminal, "USB: Device detected on hub port %u\r\n", hub_port);
                        
                        // Reset el puerto del hub
                        usb_setup_packet_t reset_req = {
                            .bmRequestType = 0x23,
                            .bRequest = 0x03,       // SET_FEATURE
                            .wValue = 4,            // PORT_RESET
                            .wIndex = hub_port,
                        ...
                        if (usb_control_transfer(device, &reset_req, NULL, 0)) {
                            terminal_printf(&main_terminal, "USB: Hub port %u reset initiated\r\n", hub_port);
                            
                            // Esperar reset (100ms)
                            for (volatile int i = 0; i < 10000000; i++);
                            
                            // Clear PORT_RESET feature
                            usb_setup_packet_t clear_reset = {
                                .bmRequestType = 0x23,
                            ...
                            if (usb_control_transfer(device, &status_req, &port_status, 4)) {
                                terminal_printf(&main_terminal, "USB: Hub port %u status after reset = 0x%08x\r\n",
                                               hub_port, port_status);
                                
                                // Bit 1 = PES (Port Enable Status)
                                if (port_status & 0x02) {
                                    terminal_printf(&main_terminal, "USB: Hub port %u enabled, enumerating device...\r\n", hub_port);
                                    // RECURSIÓN: enumerar dispositivo conectado al hub
                                    // Nota: esto es simplificado, no pasamos hub_port correctamente
    ...
    if (!usb_get_device_descriptor(device)) {
        terminal_puts(&main_terminal, "USB: Failed to get device descriptor after address set\r\n");
        usb_remove_device(device);
        return false;
    }
    ...
    if (!usb_get_config_descriptor(device, 0, config_buffer, sizeof(config_buffer))) {
        terminal_puts(&main_terminal, "USB: Failed to get configuration descriptor\r\n");
        usb_remove_device(device);
        return false;
    }
    ...
    terminal_printf(&main_terminal, "USB: Device enumerated: VID=%04x PID=%04x Class=%02x\r\n",
                   device->descriptor.idVendor, device->descriptor.idProduct,
                   device->class_code);
...
bool usb_get_device_descriptor(usb_device_t* device) {
    // Estrategia más robusta: pedir 18 bytes directamente en una sola transferencia
    // Si el dispositivo no puede manejar 18 bytes inicialmente, caerá a 8
    ...
    if (device->address == 0) {
        ...
        if ((device->speed == USB_SPEED_HIGH && max_packet != 64) ||
            ...
            terminal_printf(&main_terminal, "USB: Invalid bMaxPacketSize0=%u for speed=%u\n",
...
bool usb_get_config_descriptor(usb_device_t* device, uint8_t config_index,
    ...
    usb_setup_packet_t setup = {
        .bmRequestType = 0x80,
        .bRequest = USB_REQ_GET_DESCRIPTOR,
        .wValue = (USB_DESC_CONFIGURATION << 8) | config_index,
        .wIndex = 0,
        .wLength = length
    };
...
bool usb_set_configuration(usb_device_t* device, uint8_t config_value) {
    usb_setup_packet_t setup = {
        .bmRequestType = 0x00,  // Host to device
        ...
        if (driver->class_code == device->class_code &&
            ...
            if (driver->probe && driver->probe(device)) {
                if (driver->init && driver->init(device)) {
                    terminal_printf(&main_terminal, "Driver %s initialized successfully\r\n",
                                   driver->name);
                    return;
                }
            }
...
void usb_list_devices(void) {
    ...
    for (uint8_t i = 0; i < usb_controller_count; i++) {
        ...
        terminal_printf(&main_terminal, "Controller %u (%s):\r\n",
                       i, usb_get_speed_name(ctrl->type));
...
bool usb_clear_endpoint_halt(usb_device_t* device, uint8_t endpoint) {
    ...
    if (result) {
        ...
        terminal_printf(&main_terminal, "USB: Endpoint 0x%02x halt cleared, toggle reset to 0\n", 
                       endpoint);
    }
...
const char* usb_get_speed_name(uint8_t speed) {
    switch (speed) {
        case USB_TYPE_UHCI: return "USB 1.1 (UHCI)";
        case USB_TYPE_OHCI: return "USB 1.1 (OHCI)";
        case USB_TYPE_EHCI: return "USB 2.0 (EHCI)";
        case USB_TYPE_XHCI: return "USB 3.0 (xHCI)";
        default: return "Unknown";
    }
}