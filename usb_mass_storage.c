...
static usb_driver_t msc_driver = {.name = "USB Mass Storage",
...
bool usb_msc_probe(usb_device_t *device) {
  ...
  default:
    terminal_printf(&main_terminal,
                    "USB MSC: Unknown subclass 0x%02x, trying anyway\n",
                    device->subclass);
    break;
  }
  ...
  case 0x62: // USB Attached SCSI (UAS)
    terminal_puts(&main_terminal,
                  "USB MSC: UAS protocol (not fully supported)\n");
    break;
  ...
  default:
    terminal_printf(&main_terminal,
                    "USB MSC: Unknown protocol 0x%02x, trying anyway\n",
                    device->protocol);
    break;
  }

  terminal_printf(&main_terminal,
                  "USB MSC: Compatible device - Class:0x%02x Subclass:0x%02x "
                  "Protocol:0x%02x\n",
...
bool usb_msc_init(usb_device_t *device) {
  ...
  if (!usb_get_config_descriptor(device, 0, buffer, 255)) {
    terminal_puts(&main_terminal,
                  "USB MSC: Failed to get config descriptor\r\n");
    return false;
  }
  ...
  while (remaining > 0) {
    ...
    if (type == USB_DESC_INTERFACE) {
      ...
      if (intf->bInterfaceClass == USB_CLASS_MASS_STORAGE) {
        // Log details if weird subclass/protocol
        if (intf->bInterfaceSubClass != USB_MSC_SUBCLASS_SCSI ||
            intf->bInterfaceProtocol != USB_MSC_PROTOCOL_BBB) {
          terminal_printf(&main_terminal,
                          "USB MSC: Non-standard generic MSC found "
  ...
  // Get max LUN
  usb_setup_packet_t setup = {.bmRequestType = 0xA1, // IN, class, interface
  ...
  usb_setup_packet_t reset_setup = {.bmRequestType =
  ...
  terminal_printf(&main_terminal,
                  "USB MSC: Toggle reset - IN[%u]=0, OUT[%u]=0\r\n", ep_in_idx,
                  ep_out_idx);
...
static bool usb_msc_execute_command(usb_msc_device_t *msc, uint8_t *cbd,
                                    uint8_t cbd_len, void *data,
  ...
  // terminal_printf(&main_terminal, "USB MSC: Command - Tag=%u, DataLen=%u,
  // Dir=%s, CDB[0]=0x%02x\n",
  ...
  // Fase de datos
  if (data_len > 0) {
    // terminal_printf(&main_terminal, "USB MSC: Data phase - %u bytes,
    // dir=%s\n",
    ...
    if (!usb_bulk_transfer(msc->usb_device, data_in ? msc->ep_in : msc->ep_out,
      ...
      // terminal_puts(&main_terminal, "USB MSC: Data phase completed
      // successfully\n");
    }
  }
  ...
  if (!usb_bulk_transfer(msc->usb_device, msc->ep_in, csw_buffer, csw_size,
                         true)) {
  ...
  // terminal_printf(&main_terminal, "USB MSC: CSW - Signature=0x%08x, Tag=%u,
  // Residue=%u, Status=%u\n",
  //                csw->dCSWSignature, csw->dCSWTag, csw->dCSWDataResidue,
  //                csw->bCSWStatus);

  if (csw->dCSWSignature != CSW_SIGNATURE) {
    terminal_printf(
        &main_terminal,
        "USB MSC: Invalid CSW signature: 0x%08x (expected 0x%08x)\n",
        csw->dCSWSignature, CSW_SIGNATURE);

    terminal_puts(&main_terminal,
                  "USB MSC: Attempting bulk-only mass storage reset\n");
    usb_setup_packet_t reset_setup = {.bmRequestType = 0x21,
                                      .bRequest = 0xFF,
                                      .wValue = 0,
                                      .wIndex = 0,
                                      .wLength = 0};
  ...
  if (csw->dCSWTag != cbw.dCBWTag) {
    terminal_printf(&main_terminal,
                    "USB MSC: CSW tag mismatch: expected %u, got %u\n",
  ...
  if (csw->bCSWStatus != CSW_STATUS_GOOD) {
    terminal_printf(&main_terminal,
                    "USB MSC: Command failed, status=%u, residue=%u\n",
...
bool usb_msc_inquiry(usb_msc_device_t *msc) {
  ...
  if (!usb_msc_execute_command(msc, cmd, 6, &response, sizeof(response),
                               true)) {
    return false;
  }
...
bool usb_msc_read_capacity(usb_msc_device_t *msc) {
  ...
  if (!usb_msc_execute_command(msc, cmd, 10, &response, sizeof(response),
                               true)) {
    return false;
  }
...
bool usb_msc_read_blocks(usb_msc_device_t *msc, uint32_t lba, uint16_t count,
                         void *buffer) {
  if (!msc->initialized)
    return false;

  uint8_t cmd[10] = {SCSI_CMD_READ_10,
                     0,
                     (lba >> 24) & 0xFF,
                     (lba >> 16) & 0xFF,
                     (lba >> 8) & 0xFF,
...
bool usb_msc_write_blocks(usb_msc_device_t *msc, uint32_t lba, uint16_t count,
                          const void *buffer) {
  if (!msc->initialized)
    return false;

  uint8_t cmd[10] = {SCSI_CMD_WRITE_10,
                     0,
                     (lba >> 24) & 0xFF,
                     (lba >> 16) & 0xFF,
                     (lba >> 8) & 0xFF,
...
void usb_msc_list_devices(void) {
  ...
  for (uint8_t i = 0; i < usb_msc_device_count; i++) {
    ...
    uint64_t size_mb =
        ((uint64_t)msc->block_count * msc->block_size) / (1024 * 1024);

    terminal_printf(&main_terminal,
                    "Device %u: %u MB (%u blocks x %u bytes)\r\n", i,
                    (uint32_t)size_mb, msc->block_count, msc->block_size);
  }
...
usb_msc_device_t *usb_msc_get_device(uint8_t index) {
  if (index >= usb_msc_device_count)
    return NULL;
  return &usb_msc_devices[index];
}

uint8_t usb_msc_get_device_count(void) { return usb_msc_device_count; }