...
bool atapi_init(void) {
  ...
  if (atapi_detect_device(1, 0)) {
    terminal_puts(&main_terminal,
                  "ATAPI: Found device on secondary master\r\n");
  }
  ...
  for (uint32_t i = 0; i < atapi_device_count; i++) {
    char name[16];
    snprintf(name, sizeof(name), "atapi%d", i);
    driver_instance_t *drv = atapi_driver_create(name);
    if (drv) {
...
bool atapi_detect_device(uint8_t bus, uint8_t drive) {
  ...
  terminal_printf(
      &main_terminal,
      "ATAPI: Checking device on bus %u, drive %u: sig=0x%02x%02x\r\n", bus,
  ...
  if (lba_mid != 0x14 || lba_high != 0xEB) {
    terminal_printf(
        &main_terminal,
        "ATAPI: Not an ATAPI device (expected 0x14EB, got 0x%02x%02x)\r\n",
        lba_mid, lba_high);
    return false;
  }
  ...
  if (device->device_type != ATAPI_TYPE_CDROM) {
    terminal_printf(&main_terminal,
                    "ATAPI: Device type 0x%02x is not CDROM, rejecting\r\n",
                    device->device_type);
    return false;
  }
  ...
  terminal_printf(&main_terminal,
                  "ATAPI: Successfully initialized device: %s\r\n",
                  device->model);
...
bool atapi_identify_device(atapi_device_t *device) {
  ...
  for (int i = 39;
       i >= 0 && (device->model[i] == ' ' || device->model[i] == '\0'); i--) {
    device->model[i] = '\0';
  }
  ...
  // Trim trailing spaces
  for (int i = 19;
       i >= 0 && (device->serial[i] == ' ' || device->serial[i] == '\0'); i--) {
    device->serial[i] = '\0';
  }
  ...
  // Trim trailing spaces
  for (int i = 7;
       i >= 0 && (device->firmware[i] == ' ' || device->firmware[i] == '\0');
       i--) {
    device->firmware[i] = '\0';
  }
...
atapi_err_t atapi_request_sense(uint32_t device_id, atapi_sense_data_t *sense) {
  ...
  atapi_err_t result =
      atapi_send_packet(device, packet, sense_buffer, 18, true);
...
atapi_err_t atapi_read_capacity(uint32_t device_id, uint32_t *sector_count,
  ...
  atapi_err_t result =
      atapi_send_packet(device, packet, capacity_buffer, 8, true);

  if (result == ATAPI_ERR_NONE) {
    uint32_t last_lba = (capacity_buffer[0] << 24) |
                        (capacity_buffer[1] << 16) | (capacity_buffer[2] << 8) |
                        capacity_buffer[3];
    uint32_t block_size = (capacity_buffer[4] << 24) |
                          (capacity_buffer[5] << 16) |
...
atapi_err_t atapi_read_sectors(uint32_t device_id, uint32_t lba, uint32_t count,
                               void *buffer) {
  if (!atapi_initialized || device_id >= atapi_device_count || !buffer ||
      count == 0) {
    return ATAPI_ERR_INVALID_PARAM;
  }
  ...
  while (sectors_read < count) {
    uint32_t sectors_to_read =
        (count - sectors_read > 16) ? 16 : (count - sectors_read);
    ...
    atapi_err_t result =
        atapi_send_packet(device, packet, buf, transfer_size, true);
...
uint32_t atapi_get_device_count(void) { return atapi_device_count; }
...
void atapi_list_devices(void) {
  ...
  for (uint32_t i = 0; i < atapi_device_count; i++) {
    ...
    if (device->media_present && device->sector_count > 0) {
      uint32_t size_mb =
          (device->sector_count * device->sector_size) / (1024 * 1024);
...
const char *atapi_get_error_string(atapi_err_t error) {
  ...
  case ATAPI_ERR_IO_ERROR:
    return "I/O error";
  case ATAPI_ERR_DEVICE_NOT_READY:
    return "Device not ready";
  case ATAPI_ERR_LBA_OUT_OF_RANGE:
    return "LBA out of range";
  case ATAPI_ERR_NOT_SUPPORTED:
...
static int atapi_driver_init(driver_instance_t *drv, void *config) {
  (void)config;
  if (!drv)
    return -1;
  if (!atapi_init())
...
static int atapi_driver_start(driver_instance_t *drv) {
  if (!drv)
    return -1;
  terminal_printf(&main_terminal,
                  "ATAPI driver: Started. Found %u devices.\r\n",
                  atapi_device_count);
  return 0;
}

static int atapi_driver_stop(driver_instance_t *drv) {
  if (!drv)
    return -1;
  return 0;
}

static int atapi_driver_cleanup(driver_instance_t *drv) {
  if (!drv)
    return -1;
  atapi_cleanup();
  return 0;
...
static int atapi_driver_ioctl(driver_instance_t *drv, uint32_t cmd, void *arg) {
  if (!drv)
    return -1;

  switch (cmd) {
  case 0x4001: // List devices
    atapi_list_devices();
    return 0;
  case 0x4002: { // Get device count
    uint32_t *count = (uint32_t *)arg;
    if (count)
      *count = atapi_device_count;
    return 0;
  }
  case 0x4003: { // Eject
    uint32_t *id = (uint32_t *)arg;
    return id ? atapi_eject(*id) : -1;
  }
  default:
    return -1;
  }
...
static driver_ops_t atapi_driver_ops = {.init = atapi_driver_init,
                                        .start = atapi_driver_start,
                                        .stop = atapi_driver_stop,
                                        .cleanup = atapi_driver_cleanup,
...
static driver_type_info_t atapi_driver_type = {.type = DRIVER_TYPE_ATAPI,
                                               ...
                                               .version = "1.0.0",
                                               .priv_data_size = 0,
                                               .default_ops = &atapi_driver_ops,
                                               .validate_data = NULL,
                                               .print_info = NULL};

int atapi_driver_register_type(void) {
  return driver_register_type(&atapi_driver_type);
}
