...
unsigned char keyboard_map_scancode(uint8_t scancode, uint8_t shift, uint8_t altgr) {
...
static const int extended_keys[] = {
...
static int is_extended_key(uint8_t scancode) {
    for (uint32_t i = 0; i < sizeof(extended_keys) / sizeof(extended_keys[0]); i++) {
...
int keyboard_process_scancode(uint8_t scancode, KeyboardState* state, uint8_t* extended_flag) {
    ...
    switch (key_code) {
        ...
        case LEFT_ALT:
            ...
            return 0;
        case CAPS_LOCK:
            if (!is_release) state->caps_lock = !state->caps_lock;
            return 0;
        default:
            break;
    }
    ...
    if (state->caps_lock && !state->altgr && ((c >= 'a' && c <= 'z') || c == 0xF1)) {
