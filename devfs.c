...
static int dev_read(vfs_node_t *node, uint8_t *buf, uint32_t size,
                    uint32_t offset);
static int dev_write(vfs_node_t *node, const uint8_t *buf, uint32_t size,
                     uint32_t offset);
static int dev_readdir(vfs_node_t *dir, vfs_dirent_t *dirents, uint32_t *count,
                       uint32_t offset);
static void dev_release(vfs_node_t *node);
static int dev_getattr(vfs_node_t *node, vfs_dirent_t *attr);

static vnode_ops_t dev_vnode_ops = {.lookup = dev_lookup,
                                    .read = dev_read,
                                    .write = dev_write,
                                    .readdir = dev_readdir,
                                    .release = dev_release,
...
static vfs_node_t *create_dev_node(const char *name, int type, uint32_t id,
                                   vfs_superblock_t *sb) {
  ...
  serial_printf(COM1_BASE,
                "create_dev_node: Created %s (id=%u, type=%d) at %p\n", name,
                id, type, vn);
...
static int dev_read(vfs_node_t *node, uint8_t *buf, uint32_t size,
  ...
  if (id != DEV_NODE_COM1 && id != DEV_NODE_COM2)
    return -1;
...
static void dev_release(vfs_node_t *node) { kernel_free(node); }
...
vfs_fs_type_t devfs_type = {.name = "devfs", .mount = devfs_mount};