...
static void acpi_clear_mapping_cache(void) {
    terminal_puts(&main_terminal, "ACPI: Clearing mapping cache due to corruption\n");
    memset(mapping_cache, 0, sizeof(mapping_cache));
    cache_index = 0;
}

bool mmu_ensure_physical_accessible_cached(uint32_t phys_start, uint32_t size, uint32_t* virt_addr) {
    ...
    //terminal_printf(&main_terminal, "ACPI Cache: Request phys=0x%08x, size=%u, aligned_phys=0x%08x, aligned_size=%u\n",
    ...
    for (int i = 0; i < 8; i++) {
        if (mapping_cache[i].valid && 
            ...
            (mapping_cache[i].phys_start + mapping_cache[i].size) >= (aligned_start + aligned_size)) {
            ...
            if (verified_phys == phys_start) {
                *virt_addr = candidate_virt;
                //terminal_printf(&main_terminal, "ACPI Cache: Using cached mapping [%d]: phys=0x%08x -> virt=0x%08x (VERIFIED)\n", 
                //               i, phys_start, *virt_addr);
                return true;
            } else {
                terminal_printf(&main_terminal, "ACPI Cache: Cached mapping [%d] INVALID: virt=0x%08x maps to phys=0x%08x, expected 0x%08x\n",
    ...
    //terminal_printf(&main_terminal, "ACPI Cache: No valid cache entry, calling mmu_ensure_physical_accessible\n");
    
    if (!mmu_ensure_physical_accessible(phys_start, size, virt_addr)) {
        terminal_printf(&main_terminal, "ACPI Cache: mmu_ensure_physical_accessible FAILED for phys=0x%08x\n", phys_start);
        return false;
    }
    ...
    if (verified_phys != phys_start) {
        terminal_printf(&main_terminal, "ACPI Cache: CRITICAL - New mapping incorrect: virt=0x%08x -> phys=0x%08x, expected 0x%08x\n",
    ...
    //terminal_printf(&main_terminal, "ACPI Cache: Added to cache[%d]: phys=0x%08x, virt=0x%08x, size=%u\n",
    //               cache_index, aligned_start, mapping_cache[cache_index].virt_start, aligned_size);
...
void acpi_init(void) {
    ...
    if (!acpi_info.rsdp) {
        terminal_puts(&main_terminal, "ACPI: RSDP not found. ACPI not available.\r\n");
        return;
    }
    
    //terminal_printf(&main_terminal, "ACPI: Found RSDP at 0x%08x\r\n", (uint32_t)acpi_info.rsdp);
    //terminal_printf(&main_terminal, "ACPI: OEM ID: %.6s\r\n", acpi_info.rsdp->oem_id);
    //terminal_printf(&main_terminal, "ACPI: Revision: %u\r\n", acpi_info.rsdp->revision);
    
    // Verificar integridad del RSDP antes de continuar
    if (!acpi_validate_checksum(acpi_info.rsdp, 20)) {
        terminal_puts(&main_terminal, "ACPI: RSDP checksum invalid, ACPI disabled\r\n");
        acpi_info.rsdp = NULL;
        return;
    }
    ...
    if (acpi_info.rsdp->revision >= 2) {
        if (!acpi_validate_checksum(acpi_info.rsdp, acpi_info.rsdp->length)) {
            terminal_puts(&main_terminal, "ACPI: RSDP extended checksum invalid, ACPI disabled\r\n");
            acpi_info.rsdp = NULL;
            return;
        }
    }
    ...
    if (acpi_info.table_count == 0) {
        terminal_puts(&main_terminal, "ACPI: No valid tables found, ACPI disabled\r\n");
        acpi_info.rsdp = NULL;
        return;
    }
    ...
    for (uint32_t i = 0; i < acpi_info.table_count; i++) {
        if (acpi_info.tables[i]) {
            ...
            terminal_printf(&main_terminal, "  [%u] %.4s at 0x%08x, length=%u\r\n", 
                           i, header->signature, (uint32_t)header, header->length);
        }
    }
    ...
    if (!acpi_info.fadt) {
        terminal_puts(&main_terminal, "ACPI: FADT not found via acpi_find_table, attempting manual search\r\n");
        ...
        for (uint32_t i = 0; i < acpi_info.table_count; i++) {
            if (acpi_info.tables[i]) {
                ...
                if (!mmu_is_mapped((uint32_t)header) || !mmu_is_mapped((uint32_t)header + sizeof(acpi_sdt_header_t) - 1)) {
                    //terminal_printf(&main_terminal, "  Table [%u] not properly mapped, skipping\r\n", i);
                    continue;
                }
                ...
                if (memcmp(header->signature, "FACP", 4) == 0) {
                    //terminal_printf(&main_terminal, "  Found FACP manually at 0x%08x, length=%u\r\n", 
                                   //(uint32_t)header, header->length);
                    
                    // Verificar que toda la tabla esté mapeada
                    if (!mmu_is_mapped((uint32_t)header + header->length - 1)) {
                        terminal_puts(&main_terminal, "  FACP table not fully mapped, attempting remap\r\n");
                        
                        // Intentar remapear la tabla completa
                        uint32_t table_phys = mmu_virtual_to_physical((uint32_t)header);
                        if (table_phys == 0) {
                            terminal_puts(&main_terminal, "  Cannot determine physical address, skipping\r\n");
                            continue;
                        }
                        
                        uint32_t new_virt = 0;
                        if (!mmu_ensure_physical_accessible_cached(table_phys, header->length, &new_virt)) {
                            terminal_puts(&main_terminal, "  Failed to remap FACP table, skipping\r\n");
                            continue;
                        }
                    ...
                    if (!acpi_validate_checksum(header, header->length)) {
                        terminal_puts(&main_terminal, "  FACP checksum invalid, skipping\r\n");
                        continue;
                    }
    ...
    if (acpi_info.fadt) {
        //terminal_puts(&main_terminal, "ACPI: Parsing FADT for power management...\r\n");
        
        if (acpi_parse_fadt(acpi_info.fadt)) {
            terminal_puts(&main_terminal, "ACPI: Power management initialized successfully\r\n");
        } else {
            terminal_puts(&main_terminal, "ACPI: Failed to parse FADT, power management unavailable\r\n");
            acpi_info.fadt = NULL;
        }
    } else {
        terminal_puts(&main_terminal, "ACPI: No FADT found, power management unavailable\r\n");
    }
    
    // Marcar como inicializado solo si tenemos al menos las tablas básicas
    acpi_info.initialized = true;
    //terminal_printf(&main_terminal, "ACPI initialization complete. Version %u.0, %u tables, PM %s.\r\n",
    ...
    if (acpi_info.table_count > 0 && !acpi_info.fadt) {
        terminal_puts(&main_terminal, "ACPI: Clearing mapping cache due to FADT issues\r\n");
...
acpi_rsdp_t* acpi_find_rsdp(void) {
    ...
    //terminal_printf(&main_terminal, "ACPI: Checking EBDA pointer at 0x040E...\r\n");
    ...
    if (ebda_segment >= 0x8000 && ebda_segment < 0xA000) {
        ...
        if (mmu_ensure_physical_accessible_cached(ebda_address, 1024, &ebda_virtual)) {
            //terminal_printf(&main_terminal, "ACPI: EBDA accessible at virtual 0x%08x\r\n", ebda_virtual);
            acpi_rsdp_t* rsdp = acpi_search_rsdp_in_range((void*)ebda_virtual, 1024);
            if (rsdp) {
                //terminal_printf(&main_terminal, "ACPI: Found RSDP in EBDA at physical 0x%08x\r\n", ebda_address);
                return rsdp;
            } else {
                terminal_puts(&main_terminal, "ACPI: RSDP not found in EBDA\r\n");
            }
        } else {
            terminal_printf(&main_terminal, "ACPI: Failed to access EBDA at 0x%08x\r\n", ebda_address);
        }
    } else {
        terminal_printf(&main_terminal, "ACPI: Invalid EBDA segment 0x%04x, skipping EBDA search\r\n", ebda_segment);
    }
    
    // Buscar en BIOS ROM area
    //terminal_puts(&main_terminal, "ACPI: Searching for RSDP in BIOS ROM area (0xE0000-0xFFFFF)...\r\n");
    
    uint32_t bios_virtual = 0;
    if (mmu_ensure_physical_accessible_cached(0xE0000, 0x20000, &bios_virtual)) {
        //terminal_printf(&main_terminal, "ACPI: BIOS ROM accessible at virtual 0x%08x\r\n", bios_virtual);
        acpi_rsdp_t* rsdp = acpi_search_rsdp_in_range((void*)bios_virtual, 0x20000);
        if (rsdp) {
            //terminal_printf(&main_terminal, "ACPI: Found RSDP in BIOS ROM at physical 0x%08x\r\n", 
                           //(uint32_t)rsdp - bios_virtual + 0xE0000);
            return rsdp;
        } else {
            terminal_puts(&main_terminal, "ACPI: RSDP not found in BIOS ROM area\r\n");
        }
    } else {
...
void acpi_parse_tables(void) {
    if (acpi_info.acpi_version >= 2 && acpi_info.rsdp->xsdt_address) {
        ...
        if (xsdt_phys > 0xFFFFFFFF) {
            terminal_puts(&main_terminal, "ACPI: XSDT address too high, falling back to RSDT\r\n");
            acpi_parse_rsdt();
            return;
        }
        
        uint32_t xsdt_virt = 0;
        if (!mmu_ensure_physical_accessible_cached((uint32_t)xsdt_phys, sizeof(acpi_sdt_header_t), &xsdt_virt)) {
        ...
        // Mapear toda la XSDT
        if (!mmu_ensure_physical_accessible_cached((uint32_t)xsdt_phys, acpi_info.xsdt->header.length, &xsdt_virt)) {
            terminal_puts(&main_terminal, "ACPI: Failed to map complete XSDT\r\n");
            return;
        }
        
        if (!acpi_validate_checksum(acpi_info.xsdt, acpi_info.xsdt->header.length)) {
...
void acpi_parse_rsdt(void) {
    ...
    if (!mmu_ensure_physical_accessible_cached(rsdt_phys, sizeof(acpi_sdt_header_t), &rsdt_virt)) {
    ...
    // Mapear toda la RSDT
    if (!mmu_ensure_physical_accessible_cached(rsdt_phys, acpi_info.rsdt->header.length, &rsdt_virt)) {
    ...
    uint32_t entry_count = (acpi_info.rsdt->header.length - sizeof(acpi_sdt_header_t)) / sizeof(uint32_t);
    
    //terminal_printf(&main_terminal, "ACPI: RSDT contains %u entries\r\n", entry_count);
    
    for (uint32_t i = 0; i < entry_count && acpi_info.table_count < MAX_ACPI_TABLES; i++) {
        uint32_t table_phys = acpi_info.rsdt->sdt_pointers[i];
        
        uint32_t table_virt = 0;
        if (!mmu_ensure_physical_accessible_cached(table_phys, sizeof(acpi_sdt_header_t), &table_virt)) {
        ...
        if (!mmu_ensure_physical_accessible_cached(table_phys, table_header->length, &table_virt)) {
            terminal_printf(&main_terminal, "ACPI: Failed to map complete table %.4s\r\n", table_header->signature);
            continue;
        }
        
        if (!acpi_validate_checksum(table_header, table_header->length)) {
            terminal_printf(&main_terminal, "ACPI: Table %.4s has invalid checksum\r\n", table_header->signature);
            continue;
        }
        ...
        //               table_header->signature, acpi_get_table_name(table_header->signature));
    }
}

void acpi_parse_xsdt(void) {
    // Similar a RSDT pero con punteros de 64 bits
    uint32_t entry_count = (acpi_info.xsdt->header.length - sizeof(acpi_sdt_header_t)) / sizeof(uint64_t);
    ...
    for (uint32_t i = 0; i < entry_count && acpi_info.table_count < MAX_ACPI_TABLES; i++) {
        ...
        if (table_phys_64 > 0xFFFFFFFF) {
            terminal_printf(&main_terminal, "ACPI: Table %u address too high, skipping\r\n", i);
            continue;
        }
...
static bool acpi_parse_fadt(acpi_fadt_t* fadt) {
    //terminal_printf(&main_terminal, "ACPI: acpi_parse_fadt called with fadt=%p\r\n", fadt);
    ...
    if (fadt_phys == 0) {
        terminal_puts(&main_terminal, "ACPI: ERROR - FADT virtual address not mapped to physical memory!\r\n");
        return false;
    }
    ...
    //terminal_puts(&main_terminal, "ACPI: FADT validation passed, continuing parsing...\r\n");
    ...
    //terminal_printf(&main_terminal, "ACPI: PM1A Control: 0x%x, Status: 0x%x\r\n", 
    //               pm->pm1a_control_port, pm->pm1a_status_port);
    
    if (pm->pm1b_control_port) {
        //terminal_printf(&main_terminal, "ACPI: PM1B Control: 0x%x, Status: 0x%x\r\n", 
    ...
    if (pm->s5_sleep_type_a == 0 && pm->s5_sleep_type_b == 0) {
        ...
        //terminal_puts(&main_terminal, "ACPI: Using default S5 sleep values (7,7)\r\n");
    }
    if (fadt->header.revision >= 3) {  // ACPI 2.0+ FADT revision starts at 3
        ...
        //terminal_printf(&main_terminal, "ACPI: Found reset register (space_id=%u, addr=0x%08x, value=0x%02x)\r\n",
        //                pm->reset_reg.address_space_id, (uint32_t)pm->reset_reg.address, pm->reset_value);
    }
...
static void acpi_parse_dsdt_for_s5(void) {
    ...
    if (!mmu_ensure_physical_accessible_cached(dsdt_phys, sizeof(acpi_sdt_header_t), &dsdt_virt)) {
        return;
    }
    ...
    if (!mmu_ensure_physical_accessible_cached(dsdt_phys, dsdt_header->length, &dsdt_virt)) {
        return;
    }
            ...
            for (uint32_t j = i + 4; j < dsdt_size - 2 && j < i + 20; j++) {
                if (dsdt_data[j] == 0x12) { // Package op
                    if (j + 4 < dsdt_size) {
                        ...
                        //terminal_printf(&main_terminal, "ACPI: Found S5 sleep types: %u, %u\r\n",
...
bool acpi_enable(void) {
    if (!acpi_info.initialized || !acpi_info.fadt) {
        terminal_puts(&main_terminal, "ACPI: ACPI not initialized or FADT not found\r\n");
        return false;
    }
    ...
    if (pm->smi_command_port && pm->acpi_enable_value) {
        //terminal_printf(&main_terminal, "ACPI: Enabling ACPI via SMI (port 0x%x, value 0x%x)\r\n",
        ...
        for (int i = 0; i < 100; i++) {
            ...
            if (pm1_control & ACPI_PM1_CNT_SCI_EN) {
                pm->acpi_enabled = true;
                pm->sci_enabled = true;
                terminal_puts(&main_terminal, "ACPI: ACPI enabled successfully via SMI\r\n");
                return true;
            }
        ...
        terminal_puts(&main_terminal, "ACPI: Timeout waiting for ACPI to enable via SMI\r\n");
    }
    ...
    if (pm1_control & ACPI_PM1_CNT_SCI_EN) {
        pm->acpi_enabled = true;
        pm->sci_enabled = true;
        terminal_puts(&main_terminal, "ACPI: ACPI enabled successfully (direct)\r\n");
        return true;
    }
...
void acpi_power_off(void) {
    //terminal_printf(&main_terminal, "ACPI: Debug - initialized=%d, fadt=%p\r\n", 
    ...
    if (pm->pm1a_control_port) {
        ...
        boot_log_info("ACPI: Writing 0x%04x to PM1A port 0x%x\r\n", 
                       pm1_control, pm->pm1a_control_port);
        ...
        boot_log_warn("ACPI: ACPI power off command sent but system did not power off\r\n");
        boot_log_error();
    }
...
void acpi_reboot(void) {
    ...
    for (int i = 0; i < VFS_MAX_MOUNTS; i++) {
        if (mount_table[i] && mount_points[i][0]) {
            terminal_printf(&main_terminal, "Unmounting %s\r\n", mount_points[i]);
            serial_write_string(COM1_BASE, "Unmounting filesystem\r\n");
            if (close_fds_for_mount(mount_table[i]) != VFS_OK) {
                terminal_printf(&main_terminal, "Warning: Failed to close FDs for %s\r\n", mount_points[i]);
                serial_write_string(COM1_BASE, "Warning: Failed to close FDs\r\n");
            }
            int ret = vfs_unmount(mount_points[i]);
            if (ret != VFS_OK) {
                terminal_printf(&main_terminal, "Warning: Failed to unmount %s (error %d)\r\n", 
                               mount_points[i], ret);
                serial_write_string(COM1_BASE, "Warning: Failed to unmount\r\n");
            }
        }
    }
    ...
    terminal_puts(&main_terminal, "ACPI: Checking ACPI reset availability...\r\n");
    serial_write_string(COM1_BASE, "ACPI: Checking ACPI reset availability...\r\n");
    ...
    if (!acpi_info.initialized) {
        ...
        terminal_printf(&main_terminal, "ACPI: Version %u is too low (need 2.0+)\r\n", acpi_info.acpi_version);
        serial_write_string(COM1_BASE, "ACPI: Version too low\r\n");
    } else if (acpi_info.fadt->header.revision < 3) {
        terminal_printf(&main_terminal, "ACPI: FADT revision %u is too low (need 3+)\r\n", acpi_info.fadt->header.revision);
        serial_write_string(COM1_BASE, "ACPI: FADT revision too low\r\n");
    } else {
        acpi_pm_info_t* pm = &acpi_info.pm_info;
        if (pm->reset_reg.address == 0 || pm->reset_value == 0) {
            terminal_puts(&main_terminal, "ACPI: Reset register or value not set\r\n");
            serial_write_string(COM1_BASE, "ACPI: Reset register or value not set\r\n");
        } else {
            ...
            if (!pm->acpi_enabled) {
                ...
                if (acpi_enable()) {
                    ...
                    serial_write_string(COM1_BASE, "ACPI: Failed to enable for reset\r\n");
                }
            }

            // Perform the reset
            uint32_t addr = (uint32_t)pm->reset_reg.address; // Assume <4GB for 32-bit OS
            uint8_t value = pm->reset_value;
            switch (pm->reset_reg.address_space_id) {
                case 1: // System I/O
                    terminal_printf(&main_terminal, "ACPI: Writing 0x%02x to I/O port 0x%08x\r\n", value, addr);
                    ...
                    switch (pm->reset_reg.access_size) {
                        ...
                        case 3: // DWord access
                            ...
                            break;
                        default:
                            terminal_puts(&main_terminal, "ACPI: Unsupported access size for I/O reset\r\n");
                ...
                case 0: // System Memory
                    terminal_printf(&main_terminal, "ACPI: Writing 0x%02x to memory 0x%08x\r\n", value, addr);
                    serial_write_string(COM1_BASE, "ACPI: Writing to memory\r\n");
                    uint32_t virt_addr = 0;
                    size_t write_size = pm->reset_reg.register_bit_width / 8;
                    if (write_size == 0) write_size = 1; // Default to byte
                    if (mmu_ensure_physical_accessible_cached(addr, write_size, &virt_addr)) {
                        switch (write_size) {
                            case 1:
                            ...
                            case 4:
                                *(volatile uint32_t*)virt_addr = (uint32_t)value;
                                break;
                            default:
                                terminal_puts(&main_terminal, "ACPI: Unsupported write size for memory reset\r\n");
                                serial_write_string(COM1_BASE, "ACPI: Unsupported write size\r\n");
                                break;
                        }
                        acpi_reset_success = true;
                    } else {
                        terminal_puts(&main_terminal, "ACPI: Failed to map memory for reset\r\n");
                        serial_write_string(COM1_BASE, "ACPI: Failed to map memory\r\n");
                    }
                    break;

                default:
                    terminal_puts(&main_terminal, "ACPI: Unsupported address space for reset\r\n");
    ...
    if (!acpi_reset_success) {
        terminal_puts(&main_terminal, "ACPI: Falling back to legacy reboot methods...\r\n");
        serial_write_string(COM1_BASE, "ACPI: Falling back to legacy reboot methods...\r\n");

        // Method 1: Keyboard controller reset
        terminal_puts(&main_terminal, "ACPI: Trying keyboard controller reset...\r\n");
        serial_write_string(COM1_BASE, "ACPI: Trying keyboard controller reset...\r\n");
        uint8_t good = 0x02;
    ...
    terminal_puts(&main_terminal, "ACPI: All reboot methods failed. System halted.\r\n");
    serial_write_string(COM1_BASE, "ACPI: All reboot methods failed. System halted.\r\n");
    while (1) {
        __asm__ volatile("cli; hlt");
    }
}
...
void acpi_suspend(void) {
    ...
    terminal_puts(&main_terminal, "ACPI: Suspend to RAM not fully implemented\r\n");
    terminal_puts(&main_terminal, "ACPI: This would require saving system state\r\n");
    ...
    acpi_pm_info_t* pm = &acpi_info.pm_info;
    if (pm->pm1a_control_port) {
        terminal_printf(&main_terminal, "ACPI: Would use PM1A port 0x%x for suspend\r\n", 
                       pm->pm1a_control_port);
    }
}
...
void acpi_list_tables(void) {
    ...
    for (uint32_t i = 0; i < acpi_info.table_count; i++) {
        ...
        terminal_printf(&main_terminal, "%.4s: %s\r\n", 
                       header->signature, acpi_get_table_name(header->signature));
        ...
        terminal_printf(&main_terminal, "  OEM: %.6s %.8s\r\n", 
                       header->oem_id, header->oem_table_id);
        terminal_puts(&main_terminal, "\r\n");
    }
