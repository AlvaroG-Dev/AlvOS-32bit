...
mbr_err_t mbr_write(disk_t *disk, const mbr_t *mbr) {
    ...
    terminal_printf(&main_terminal, "MBR: MBR written and verified successfully\n");
    return MBR_OK;
}
...
mbr_err_t mbr_install_bootcode(disk_t *disk, const uint8_t *boot_code, uint32_t size) {
    if (!disk || !boot_code || size == 0 || size > MBR_BOOT_CODE_SIZE) {
        terminal_printf(&main_terminal, "MBR: Invalid boot code (size=%u, max=%u)\n", 
                       size, MBR_BOOT_CODE_SIZE);
...
bool mbr_verify_signature(const mbr_t *mbr) {
    return mbr && mbr->signature == BOOT_SIGNATURE;
}
...
mbr_err_t vbr_write(disk_t *disk, uint64_t partition_lba, const vbr_fat32_t *vbr) {
    ...
    if (err != DISK_ERR_NONE || memcmp(vbr, &verify_vbr, sizeof(vbr_fat32_t)) != 0) {
...
bool vbr_verify_fat32(const vbr_fat32_t *vbr) {
    ...
    // Check signature
    if (vbr->signature != BOOT_SIGNATURE) return false;
    ...
    // Sanity checks
    if (vbr->bytes_per_sector != 512) return false;
    if (vbr->sectors_per_cluster == 0) return false;
    if (vbr->num_fats == 0) return false;
    if (vbr->root_entries != 0) return false; // FAT32 specific
    if (vbr->total_sectors_16 != 0) return false; // FAT32 specific
