...
typedef struct {
  ...
  uint8_t ep_toggles[32]; // Added: Index = (ep_num & 0x0F) + (is_in ? 16 : 0);
                          // 0=OUT, 1=IN directions
} usb_device_t;
...
bool usb_bulk_transfer(usb_device_t *device, uint8_t endpoint, void *data,
...
struct driver_instance *usb_driver_create(const char *name);
