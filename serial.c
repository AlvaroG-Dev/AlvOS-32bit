...
void serial_init(void) {
  ...
  driver_instance_t *drv = serial_driver_create("com_ports");
  if (drv) {
...
/**
 * Handler específico para IRQ 3 o 4 (serial). Pásale 3 ó 4 directamente desde
 * el stub ASM.
 */
...
void serial_printf(uint16_t port, const char *fmt, ...) {
  ...
  if (len < 0 || len >= (int)sizeof(buf)) {
    serial_write_string(port,
                        "serial_printf: Format error or buffer overflow\n");
    return;
  }
...
static int serial_driver_ioctl(driver_instance_t *drv, uint32_t cmd,
  ...
  // Commands could be: write string, set baud rate, etc.
  switch (cmd) {
