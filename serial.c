...
// ============================================================================
static int serial_chardev_read(uint8_t *buf, uint32_t size, uint32_t offset,
                               void *priv) {
  (void)offset;
  if (!priv || !buf || size == 0)
    return 0;
  ...
  int available =
      (*rx_head - *rx_tail + SERIAL_RX_QUEUE_SIZE) % SERIAL_RX_QUEUE_SIZE;
...
static int serial_chardev_write(const uint8_t *buf, uint32_t size,
                                uint32_t offset, void *priv) {
  (void)offset;
  if (!priv || !buf || size == 0)
    return 0;
...
static int serial_chardev_ioctl(uint32_t cmd, void *arg, void *priv) {
  ...
  case 0x5401: // TCGETS (get terminal attributes)
    if (arg)
      memset(arg, 0, 60); // Tamaño aproximado de termios
    return 0;
  ...
  case 0x541B: // FIONREAD (bytes disponibles)
    if (arg) {
      volatile int *rx_head =
          (port == COM1_BASE) ? &rx_head_com1 : &rx_head_com2;
      volatile int *rx_tail =
          (port == COM1_BASE) ? &rx_tail_com1 : &rx_tail_com2;
      int available =
          (*rx_head - *rx_tail + SERIAL_RX_QUEUE_SIZE) % SERIAL_RX_QUEUE_SIZE;
      *(int *)arg = available;
    }
...
static chardev_ops_t serial_ops = {.read = serial_chardev_read,
...
int serial_available(uint16_t port) {
  if (port == COM1_BASE) {
    return (rx_head_com1 - rx_tail_com1 + SERIAL_RX_QUEUE_SIZE) %
           SERIAL_RX_QUEUE_SIZE;
  } else if (port == COM2_BASE) {
    return (rx_head_com2 - rx_tail_com2 + SERIAL_RX_QUEUE_SIZE) %
           SERIAL_RX_QUEUE_SIZE;
  }
  return 0;
}
...
// ============================================================================
static chardev_t *create_serial_chardev(uint16_t port, const char *name,
                                        uint8_t minor) {
...
// ============================================================================
static int serial_vfs_read(vfs_node_t *node, uint8_t *buf, uint32_t size,
                           uint32_t offset) {
...
static int serial_vfs_write(vfs_node_t *node, const uint8_t *buf, uint32_t size,
                            uint32_t offset) {
  chardev_t *dev = (chardev_t *)node->fs_private;
...
static vnode_ops_t serial_vnode_ops = {.read = serial_vfs_read,
...
int serial_create_devfs_nodes(void) {
      ...
      if (node) {
        ...
        if (dev) {
          ...
          terminal_printf(&main_terminal,
                          "Configured VFS node for %s with chardev\r\n",
                          devices[i]);
        } else {
          terminal_printf(&main_terminal,
                          "Warning: chardev not found for %s\r\n", devices[i]);
        }
        node->refcount--;
  ...
  terminal_printf(&main_terminal,
                  "Serial VFS nodes created with chardev backend\r\n");
  return 0;
}
...
void serial_dump_status(uint16_t port) {
  ...
  if (lsr & UART_LSR_DR)
    terminal_printf(&main_terminal, "DATA ");
  if (lsr & UART_LSR_THRE)
    terminal_printf(&main_terminal, "THRE ");
  terminal_printf(&main_terminal, ")\r\n");

  terminal_printf(&main_terminal, "MSR: 0x%02x\r\n", msr);
  terminal_printf(&main_terminal, "IER: 0x%02x (", ier);
  if (ier & UART_IER_RX)
    terminal_printf(&main_terminal, "RX ");
  if (ier & UART_IER_THRE)
    terminal_printf(&main_terminal, "THRE ");
  terminal_printf(&main_terminal, ")\r\n");
  ...
  int rx_avail =
      (*rx_head - *rx_tail + SERIAL_RX_QUEUE_SIZE) % SERIAL_RX_QUEUE_SIZE;
  int tx_avail =
      (*tx_head - *tx_tail + SERIAL_TX_QUEUE_SIZE) % SERIAL_TX_QUEUE_SIZE;

  terminal_printf(&main_terminal,
                  "RX Queue: %d/%d bytes (head=%d, tail=%d)\r\n", rx_avail,
                  SERIAL_RX_QUEUE_SIZE, *rx_head, *rx_tail);
  terminal_printf(
      &main_terminal, "TX Queue: %d/%d bytes (head=%d, tail=%d, busy=%d)\r\n",
...
void cmd_serial_test(const char *args) {
  ...
  if (!token) {
    terminal_printf(&main_terminal,
                    "Error: Se requiere puerto (com1 o com2)\r\n");
    return;
  }
  ...
  if (strcmp(com_port, "com1") == 0) {
    ...
    terminal_printf(&main_terminal,
                    "Error: Puerto inválido '%s'. Use 'com1' o 'com2'\r\n",
                    com_port);
    return;
  }
...
static int serial_driver_ioctl(driver_instance_t *drv, uint32_t cmd,
                               void *arg) {
  if (!drv)
    return -1;
...
static driver_ops_t serial_driver_ops = {.init = serial_driver_init,
...
static driver_type_info_t serial_driver_type = {.type = DRIVER_TYPE_SERIAL,
                                                .type_name = "serial",
                                                .version = "1.0.2",
                                                .priv_data_size = 0,
                                                .default_ops =
