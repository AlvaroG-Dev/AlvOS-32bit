...
bool apic_init(void) {
  ...
  if (!acpi_is_supported()) {
    terminal_puts(&main_terminal, "APIC: ACPI not available\r\n");
    __asm__ __volatile__("push %0\n\tpopf" : : "r"(flags));
    return false;
  }
  ...
  // 4. Mapear Local APIC (CRÍTICO: asegurar mapeo)
  if (!apic_info.lapic_base_phys) {
    terminal_puts(&main_terminal, "APIC: No Local APIC base address found\r\n");
    __asm__ __volatile__("push %0\n\tpopf" : : "r"(flags));
    return false;
  }

  // MAPEAR REGIÓN DEL LOCAL APIC (4096 bytes es suficiente)
  if (!mmu_ensure_physical_accessible(apic_info.lapic_base_phys, PAGE_SIZE,
                                      &apic_info.lapic_base_virt)) {
    terminal_printf(&main_terminal,
                    "APIC: Failed to map Local APIC at phys=0x%08x\r\n",
                    apic_info.lapic_base_phys);
    __asm__ __volatile__("push %0\n\tpopf" : : "r"(flags));
    return false;
  }

  terminal_printf(&main_terminal,
                  "APIC: Local APIC at phys=0x%08x, virt=0x%08x\r\n",
  ...
  for (uint8_t i = 0; i < apic_info.io_apic_count; i++) {
    ...
    if (!mmu_ensure_physical_accessible(apic_info.io_apics[i].base_address,
      ...
      // Continuar aunque falle uno, pero marcarlo
      apic_info.io_apics[i].base_address = 0;
      continue;
    }
  ...
  while (entry_ptr < end_ptr) {
    ...
    case MADT_TYPE_INTERRUPT_OVERRIDE: {
      ...
      if (apic_info.override_count < 24) {
        ...
        apic_info.overrides[apic_info.override_count].flags = override->flags;
...
void lapic_write(uint32_t reg, uint32_t value) {
  volatile uint32_t *addr =
      (volatile uint32_t *)(apic_info.lapic_base_virt + reg);
  *addr = value;
}
...
uint8_t lapic_get_id(void) { return (lapic_read(LAPIC_ID) >> 24) & 0xFF; }
...
uint32_t apic_irq_to_gsi(uint8_t irq) {
  // Buscar override específico para VirtualBox
  for (uint8_t i = 0; i < apic_info.override_count; i++) {
    if (apic_info.overrides[i].irq_source == irq) {
      terminal_printf(&main_terminal, "APIC: IRQ %u -> GSI %u (override)\r\n",
                      irq, apic_info.overrides[i].gsi);
      return apic_info.overrides[i].gsi;
    }
  }

  // Sin override, asumir IRQ = GSI (para compatibilidad)
  // NOTA: En VirtualBox con IO APIC, esto es INCORRECTO para IRQ 0
  // Pero puede funcionar para otros IRQs
  return irq;
}
...
uint32_t lapic_timer_calibrate(void) {
  terminal_puts(&main_terminal,
                "APIC: Calibrating Local APIC timer using PIT...\r\n");

  // 1. Configurar PIT Canal 2 (usado para speaker, pero seguro para timing)
  // Mode 0 (Interrupt on Terminal Count), LSB/MSB
  outb(0x43, 0xB0);

  // 2. Queremos esperar 10ms.
  // Frecuencia PIT = 1.193182 MHz. 10ms = ~11931 ticks.
  uint16_t target_pit_ticks = 11931;
  outb(0x42, target_pit_ticks & 0xFF);
  outb(0x42, (target_pit_ticks >> 8) & 0xFF);

  // 3. Encender el Speaker/Gate 2 (bit 0 de 0x61) para arrancar el conteo
  uint8_t tmp = inb(0x61);
  outb(0x61, tmp | 1);

  // 4. Iniciar APIC Timer (Valor máximo, divisor 16)
  lapic_write(LAPIC_TIMER_DCR, LAPIC_TIMER_DIV_16);
  lapic_write(LAPIC_TIMER_ICR, 0xFFFFFFFF);

  // 5. Esperar a que el PIT termine (Polling bit 5 de puerto 0x61 - Output)
  // En modo 0, la salida se pone en ALTO cuando termina el conteo.
  while ((inb(0x61) & 0x20) == 0)
    ;

  // 6. Detener APIC Timer inmediatamente
  uint32_t current_apic_count = lapic_read(LAPIC_TIMER_CCR);
  lapic_write(LAPIC_LVT_TIMER, LAPIC_LVT_MASKED); // Stop

  // 7. Apagar Gate 2
  outb(0x61, tmp & 0xFE);

  // 8. Calcular ticks
  uint32_t ticks_passed = 0xFFFFFFFF - current_apic_count;

  // Ticks pasados en 10ms * 100 = Ticks en 1 segundo (pero usamos div 16)
  // Queremos ticks/ms para la variable global
  uint32_t ticks_per_ms = ticks_passed / 10;

  terminal_printf(&main_terminal, "APIC: Calibration result: %u ticks/ms\r\n",
                  ticks_per_ms);
...
void lapic_timer_init(uint32_t frequency_hz) {
  ...
  // ✅ Guardar flags y deshabilitar interrupciones durante configuración
  uint32_t flags;
  __asm__ __volatile__("pushf\n\tcli\n\tpop %0" : "=r"(flags));
  ...
  if (!apic_info.lapic_enabled) {
    terminal_puts(&main_terminal,
                  "APIC: Local APIC not enabled, enabling now\r\n");
    apic_enable();

    // Verificar después de habilitar
    if (!apic_info.lapic_enabled) {
      terminal_puts(&main_terminal, "APIC: Failed to enable Local APIC\r\n");
      __asm__ __volatile__("push %0\n\tpopf" : : "r"(flags));
      return;
    }
  }

  // ✅ Calibrar SIEMPRE si no está calibrado
  if (apic_info.timer_ticks_per_ms == 0) {
    terminal_puts(&main_terminal, "APIC: Calibrating timer...\r\n");
    uint32_t calibrated_ticks = lapic_timer_calibrate();

    if (calibrated_ticks == 0) {
      terminal_puts(&main_terminal, "APIC: Calibration failed\r\n");
      __asm__ __volatile__("push %0\n\tpopf" : : "r"(flags));
      return;
    }
  ...
  uint8_t divisor_config = LAPIC_TIMER_DIV_16; // Por defecto
  ...
  if (frequency_hz >= 1000) {
    ...
    // Si initial_count es muy pequeño, usar divisor más pequeño
    if (initial_count < 100) {
      divisor_config = LAPIC_TIMER_DIV_1;
      initial_count = (ticks_per_second / frequency_hz) * 16;
    }
  } else {
    // Baja frecuencia: usar milisegundos
    uint32_t ms_per_interrupt = 1000 / frequency_hz;
    initial_count = (apic_info.timer_ticks_per_ms * ms_per_interrupt);
  }

  // ✅ Validación robusta
  if (initial_count == 0) {
    terminal_puts(&main_terminal, "APIC: Initial count too low, adjusting\r\n");
    initial_count = 1000;
  }
  ...
  // ✅ Vector 32 (mismo que PIC para compatibilidad)
  uint32_t vector = 32;

  // ✅ Configurar LVT Timer: vector + periódico + NO masked
  uint32_t lvt_value = vector | LAPIC_LVT_TIMER_PERIODIC;
  lvt_value &= ~LAPIC_LVT_MASKED; // Asegurar que NO esté masked

  lapic_write(LAPIC_LVT_TIMER, lvt_value);

  // ✅ Configurar initial count (debe ser después de LVT)
  ...
  if (lvt_check & LAPIC_LVT_MASKED) {
    terminal_puts(&main_terminal,
                  "APIC: ERROR - Timer is masked! Fixing...\r\n");
    lvt_check &= ~LAPIC_LVT_MASKED;
    lapic_write(LAPIC_LVT_TIMER, lvt_check);
  }

  // ✅ Esperar y verificar que está contando
  ...
  if (ccr_initial == ccr_after) {
    terminal_puts(&main_terminal,
                  "APIC: WARNING - Timer might not be counting\r\n");
    terminal_printf(&main_terminal, "  CCR before: %u, after: %u\r\n",
    ...
    ccr_after = lapic_read(LAPIC_TIMER_CCR);

    if (ccr_initial == ccr_after) {
      terminal_puts(&main_terminal,
                    "APIC: ERROR - Timer definitely not counting\r\n");
    } else {
      terminal_printf(&main_terminal, "APIC: Timer now counting (%u -> %u)\r\n",
                      ccr_initial, ccr_after);
    }
  } else {
    terminal_printf(&main_terminal, "APIC: Timer is counting! (%u -> %u)\r\n",
                    ccr_initial, ccr_after);
  }
  ...
  terminal_puts(&main_terminal, "APIC: Timer initialization complete\r\n");
}
...
bool apic_verify_state(void) {
  ...
  if (id != apic_info.lapic_id) {
    terminal_printf(&main_terminal,
                    "APIC: WARNING - ID mismatch (expected %u, got %u)\r\n",
...
void apic_print_info(void) {
  ...
  for (uint8_t i = 0; i < apic_info.io_apic_count; i++) {
    terminal_printf(&main_terminal,
                    "  [%u] ID=%u, Base=0x%08x, GSI Base=%u, "
                    "Max Entries=%u\r\n",
                    i, apic_info.io_apics[i].io_apic_id,
  ...
  for (uint8_t i = 0; i < apic_info.local_apic_count; i++) {
    terminal_printf(
        &main_terminal, "  [%u] Processor=%u, APIC ID=%u, Enabled=%s\r\n", i,
        apic_info.local_apics[i].processor_id, apic_info.local_apics[i].apic_id,
  ...
  if (apic_info.override_count > 0) {
    ...
    for (uint8_t i = 0; i < apic_info.override_count; i++) {
      terminal_printf(
          &main_terminal, "  IRQ %u -> GSI %u (Active %s, %s-triggered)\r\n",
          apic_info.overrides[i].irq_source, apic_info.overrides[i].gsi,
