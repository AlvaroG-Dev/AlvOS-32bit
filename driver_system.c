...
static void remove_from_list(driver_instance_t *drv) {
  if (!drv || !driver_system.driver_list)
    return;
...
int driver_system_init(void) {
  ...
  if (keyboard_driver_register_type() != 0) {
    terminal_printf(&main_terminal,
                    "WARNING: Failed to register keyboard driver type\r\n");
  }

  if (mouse_driver_register_type() != 0) {
    terminal_printf(&main_terminal,
                    "WARNING: Failed to register mouse driver type\r\n");
  }

  if (serial_driver_register_type() != 0) {
    terminal_printf(&main_terminal,
                    "WARNING: Failed to register serial driver type\r\n");
  }

  if (sata_disk_driver_register_type() != 0) {
    terminal_printf(&main_terminal,
                    "WARNING: Failed to register SATA disk driver type\r\n");
  }

  if (pci_driver_register_type() != 0) {
    terminal_printf(&main_terminal,
                    "WARNING: Failed to register PCI driver type\r\n");
  }

  if (atapi_driver_register_type() != 0) {
    terminal_printf(&main_terminal,
                    "WARNING: Failed to register ATAPI driver type\r\n");
  }

  if (usb_driver_register_type() != 0) {
    terminal_printf(&main_terminal,
                    "WARNING: Failed to register USB driver type\r\n");
  }

  terminal_printf(&main_terminal,
                  "Driver system initialized with %d driver types\r\n",
...
int driver_register_type(const driver_type_info_t *type_info) {
  ...
  for (int i = 0; i < driver_system.type_count; i++) {
    if (driver_system.type_registry[i].type == type_info->type) {
      terminal_printf(&main_terminal,
                      "WARNING: Driver type %d already registered\r\n",
                      type_info->type);
      return -1;
    }
  }
...
driver_instance_t *driver_create(driver_type_t type, const char *name) {
  ...
  if (!drv) {
    terminal_printf(&main_terminal,
                    "ERROR: Failed to allocate driver instance\r\n");
    return NULL;
  }
  ...
  if (type_info->priv_data_size > 0) {
    drv->priv_data = kernel_malloc(type_info->priv_data_size);
    if (!drv->priv_data) {
      terminal_printf(&main_terminal,
    ...
    memset(drv->priv_data, 0, type_info->priv_data_size);
  }
...
int driver_init(driver_instance_t *drv, void *config) {
  ...
  if (drv->state != DRIVER_STATE_UNLOADED &&
      drv->state != DRIVER_STATE_LOADED) {
    terminal_printf(&main_terminal,
                    "ERROR: Driver %s not in correct state for init\r\n",
                    drv->name);
    return -1;
  }
...
void *driver_load_binary_file(const char *filename, size_t *out_size) {
  ...
  if (!file_data) {
    terminal_printf(&main_terminal,
                    "ERROR: Cannot allocate %u bytes for driver file\r\n",
                    (uint32_t)total_size);
    vfs_close(fd);
    return NULL;
  }
  ...
  while (bytes_remaining > 0) {
    bytes_read = vfs_read(fd, data_ptr,
                          bytes_remaining > sizeof(buffer) ? sizeof(buffer)
                                                           : bytes_remaining);
    if (bytes_read <= 0) {
      terminal_printf(&main_terminal, "ERROR: Read failed at %u/%u bytes\r\n",
                      (uint32_t)(total_size - bytes_remaining),
                      (uint32_t)total_size);
  ...
  *out_size = total_size;
  ...
  terminal_printf(&main_terminal,
                  "Successfully loaded driver file %s (%u bytes)\r\n", filename,
                  (uint32_t)total_size);
  return file_data;
}
...
void driver_list_all(void) {
  ...
  while (current) {
    ...
    terminal_printf(&main_terminal, "%d. %s [%s] - %s (%s)\r\n", ++count,
                    current->name, state_str,
                    current->type_info ? current->type_info->type_name
                                       : "Unknown",
