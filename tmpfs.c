...
static int tmp_read(vfs_node_t *node, uint8_t *buf, uint32_t size,
                    uint32_t offset);
static int tmp_write(vfs_node_t *node, const uint8_t *buf, uint32_t size,
                     uint32_t offset);
static int tmp_readdir(vfs_node_t *node, vfs_dirent_t *buf, uint32_t *count,
                       uint32_t offset);
static void tmp_release(vfs_node_t *node);
static int tmp_unlink(vfs_node_t *parent, const char *name);

static vnode_ops_t tmp_vnode_ops = {.lookup = tmp_lookup,
                                    .create = tmp_create,
...
static int tmp_add_child(tmp_node_t *parent, tmp_node_t *child) {
  tmp_node_t **newarr = (tmp_node_t **)kernel_malloc(sizeof(tmp_node_t *) *
...
static int tmp_read(vfs_node_t *node, uint8_t *buf, uint32_t size,
  ...
  if (offset >= tn->size) {
    // log_message(LOG_INFO, "[TMPFS_READ] Offset beyond file size, returning
    // 0\n");
    return 0; // EOF
  }
...
static int tmp_readdir(vfs_node_t *node, vfs_dirent_t *buf, uint32_t *count,
  ...
  for (uint32_t i = offset;
       i < (uint32_t)tn->child_count && entries_written < max_entries; i++) {
...
static void tmp_release(vfs_node_t *node) { kernel_free(node); }
  ...
  for (int i = 0; i < pt->child_count; i++) {
    if (strcmp(pt->children[i]->name, name) == 0) {
      ...
      if (child->type == VFS_NODE_DIR && child->child_count > 0) {
        return -1; // No eliminar directorios no vacíos (como rm estándar sin
                   // -r)
      }

      // Liberar recursos
      if (child->data)
        kernel_free(child->data);
      if (child->children)
        kernel_free(child->children);
      kernel_free(child);
...
vfs_fs_type_t tmpfs_type = {.name = "tmpfs", .mount = tmpfs_mount};