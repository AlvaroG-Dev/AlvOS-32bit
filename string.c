...
long strtol(const char *nptr, char **endptr, int base) {
    ...
    while (*nptr == ' ' || *nptr == '\t') nptr++;  // Saltar espacios
    ...
    while (*format && written < size - 1) {
        ...
        switch (*format) {
            case 'c':
                ...
                if (written < size - 1) str[written++] = c;
                if (left_align && width > 1) {
                    for (int i = 0; i < width - 1 && written < size - 1; i++) {
                        str[written++] = ' ';
                    }
...
size_t strcspn(const char *str1, const char *str2) {
    ...
    for (s1 = str1; *s1 != '\0'; ++s1) {
        ...
        for (s2 = str2; *s2 != '\0'; ++s2) {
            if (*s1 == *s2) {
                return s1 - str1;  // Si encontramos una coincidencia, retornamos la longitud hasta el carácter coincidente
            }
        }
    }

    // Si no se encuentra ninguna coincidencia, retornamos la longitud completa de `str1`
    return s1 - str1;
}
...
static int strtoi_base(const char *str, char **endptr, int base) {
    ...
    while (*str) {
        int digit;
        if (*str >= '0' && *str <= '9') digit = *str - '0';
        else if (*str >= 'a' && *str <= 'z') digit = 10 + (*str - 'a');
        else if (*str >= 'A' && *str <= 'Z') digit = 10 + (*str - 'A');
        else break;
